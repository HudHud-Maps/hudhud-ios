...
public struct ManeuverInstructionView<ManeuverView: View>: View {
    ...
    public var body: some View {
        HStack {
            self.maneuverView
                .frame(width: 64)
                .foregroundStyle(self.theme.iconTintColor)

            VStack(alignment: .leading) {
                if let distanceToNextManeuver {
                    Text("\(self.distanceFormatter.string(for: distanceToNextManeuver) ?? "")")
                        .font(self.theme.distanceFont)
                        .foregroundStyle(self.theme.distanceColor)
                }

                Text(self.text)
                    .lineLimit(2)
                    .multilineTextAlignment(.leading)
                    .font(self.theme.instructionFont)
                    .foregroundStyle(self.theme.instructionColor)
            }
