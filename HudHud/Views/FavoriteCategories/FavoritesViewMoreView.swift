    ...
    var body: some View {
        NavigationStack {
            VStack(alignment: .leading) {
                ...
                Section { // show my favorites
                    ...
                    .confirmationDialog("action", isPresented: self.$actionSheetShown) {
                        Button("Edit") {
                            ...
                            self.sheetStore.show(.editFavoritesForm(item: item, favoriteItem: self.clickedFavorite))
                        }
                ...
                Section("Suggestions") {
                    RecentSearchResultsView(searchStore: self.searchStore, searchType: .favorites, sheetStore: self.sheetStore)
                    Spacer()
                }
                Spacer()
            }
