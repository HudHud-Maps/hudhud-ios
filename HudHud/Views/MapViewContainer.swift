...
@MainActor
final class MapViewContainerStore: ObservableObject {
    ...
    init(
        ...
        self._mapStore = ObservedObjectChild(wrappedValue: mapStore)
    ...
    func updateMapStyle(_ style: MLNStyle?) {
        if let style {
            self.navigationVisualization.intilize(style: style)
        }
...
//        self.isSheetShown = true
        ...
        if let coordinates = self.lastLocation?.coordinate {
...
struct MapViewContainer: View {
    ...
    @ObservedObject private var store: MapViewContainerStore
    ...
    var body: some View {
        NavigationStack {
            DynamicallyOrientingNavigationView(
                makeViewController: self.store.mapViewController,
                ...
                locationProviding: self.store.navigationVisualization.locationprovider,
                navigationState: self.store.navigationVisualization.navigationState,
                showZoom: false
            ) {} makeMapContent: {
...
//                            mapViewController.mapView.locationManager = nil
                        ...
                        .onTapMapGesture(on: MapLayerIdentifier.tapLayers) { context, _ in
...
//                        .cameraModifierDisabled(self.routingStore.navigatingRoute != nil) // TODO: -
                        .onMapViewPortUpdate { _ in
//                            store.mapStore.mapViewPort = viewPort
                        }
            ...
            .onChange(of: self.store.navigationVisualization.navigationState) { oldValue, newValue in
                if newValue != oldValue {
                    self.isSheetShown = !self.store.navigationVisualization.isNavigating
                }
            }
            .task {
                guard !self.didFocusOnUser else { return }
