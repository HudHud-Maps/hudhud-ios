        ...
        case .ok(let okResponse):
            ...
            case .json(let jsonResponse):
                let resolvedItem: [ResolvedItem] = jsonResponse.data.compactMap { item in
                    ...
                    return ResolvedItem(id: item.id, title: item.name, subtitle: item.category, category: item.category, symbol: .pin, type: .hudhud, coordinate: CLLocationCoordinate2D(latitude: item.coordinates.lat, longitude: item.coordinates.lon), color: Color(.systemRed), phone: nil, website: nil, rating: item.rating, ratingCount: ratingCount, trendingImage: item.trending_image_url)
                }
                    return resolvedItem
